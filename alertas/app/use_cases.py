...
from .catalog import emails_cliente, nombre_cliente
...
def ejecutar_modo_cliente(
    ...
    destinatarios = emails_cliente(rfc, clientes)
    if not destinatarios:
        logger.warning("%s no tiene emails configurados en clientes.json", nombre)
        return

    year, month = periodo.split("-")
    mes_label = MESES_ES.get(int(month), month).capitalize()
    asunto = f"Reporte Mensual CFDI - {nombre} - {mes_label} {year}"
    html = render_html_cliente(rfc, periodo, alertas_cliente, datos, clientes)
